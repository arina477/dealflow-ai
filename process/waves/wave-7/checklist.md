...
- [~] C-2 Deploy & verify — **V-3 shared-timestamp re-verify @ e3dd9b7: REJECTED (head-ci-cd)**. Shared timestamp fix (e3dd9b7) is CORRECT & 2/3 screens now render: workspace /sourcing renders companies (live headless-chromium DOM: Acme/Delta/Bright Horizon/Epsilon, 6 rows, emptyState:false) AND /compliance/settings renders 4 seeded rules (compliance-rules.ts fix confirmed with a PG-wire-timestamped approval_required rule). Deploy e3dd9b7 SUCCESS both services (web 13a28a77 / api 83012c67 via GIT_SHA variableUpsert auto-redeploy, neither SKIPPED); /health==e3dd9b7 on api's own domain; web deployment meta.commitHash==e3dd9b7 (web NOT stale). Armed rollback captured (api 599542b3 / web e520f0a1, unused). Regression: create 201 / dup 409 / bad-key 400 / login 200 / /health ok — all PASS. **BUT deep-screen /sourcing/companies STILL FAILS**: live DOM "No companies yet" with 4 companies in DB. Root cause is NOT the timestamp bug (fixed) — it is a SECOND independent defect: the API emits a `connectionIds` field per company; the deep-screen SSR parse extends a .strict() companySchema that declares no connectionIds → both safeParse reject (unrecognized_keys:['connectionIds']) → fetchCompanies returns [] → empty. Empirically reproduced against the live payload w/ the exact deployed zod schema. Iron Law tripped. Canary skipped (0 DAU). Cleanup: 0 companies/connections, seeded rule deleted, temp proxy 9ffbdca2 deleted, creds scrubbed. → REWORK_B-block (reconcile shared companySchema w/ API connectionIds OR make deep-screen parse tolerant of API-added keys; add deep-screen contract test feeding the REAL API shape incl. connectionIds), then re-run C-2. [Prior 3e2042f render-fix re-verify REJECTED for the timestamp; this re-verify cleared the timestamp but surfaced the connectionIds strict-key defect underneath.]
