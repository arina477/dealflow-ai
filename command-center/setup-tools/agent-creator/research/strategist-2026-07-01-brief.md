...
The strategist lens evaluates whether a decision advances the founder's live strategic bets and the product's long-term position, or whether it drifts, dilutes, or contradicts them. It asks: does this move strengthen DealFlow AI's differentiated position (compliance-first, integrated sourcing→matching→outreach) against stitched-together incumbents, or does it chase a distraction? It weighs opportunity cost, sequencing against the roadmap, strategic coherence across waves, and whether short-term wins mortgage long-term direction. It is the seat most alert to scope decisions that are locally reasonable but strategically off-course, and to bet-misalignment that the execution-focused seats will not notice.
...
## Role Focus
Weight research toward: how a senior strategist catches things this seat is *uniquely positioned* to catch — i.e., the failure modes the OTHER six BOARD seats systematically miss; the patterns this lens recognizes that generalists don't; the real-case precedent that grounds this seat's rationale.
...
What is the strategist lens? What does it explicitly evaluate? What does it NOT evaluate (where it ABSTAINS)? What separates a great application of this lens from a mediocre one? What kind of decision benefits MOST from this lens being applied rigorously?
...
Failure modes the OTHER six BOARD seats systematically miss but strategist should catch. Per mode:
- Name: `<short>`
  Pattern: `<what consistently happens>`
  Why other lenses miss it: `<one sentence>`
  Cost when it lands: `<concrete impact>`
  strategist's catch: `<what strategist sees that surfaces it>`

### §5 HARD-STOP TRIGGERS — 4-8 triggers
Conditions under which strategist MUST emit `HARD-STOP: must be human` regardless of vote math. Per trigger:
...
- Case: `<company / project — short label>`
  ...
  Lesson: `<what this teaches the strategist lens>`
  Source: `<link>`
...
## Source Quality
1. **PRACTITIONER** — engineering-leadership and product-strategy essays, post-mortems with named cases, public retrospectives, conference talks walking through real strategic decisions.
2. **CASE_STUDY** — documented company-decision retrospectives (Stripe Press, Shopify Engineering, AWS post-mortems, Cloudflare blog, fintech/M&A-tech company retrospectives).
3. **OFFICIAL** — methodology canonical sources where relevant to day-to-day decision substance.
4. **BOOK** — books authored by people who have done this kind of strategic evaluation at credible scale.
