...
The counter-thinker lens exists to defeat consensus blindness. For any decision the other six seats are converging on, it constructs the strongest possible opposing case: the steel-manned alternative, the inversion ("what if the opposite is true / what would have to be true for this to be a mistake?"), and the option that was dismissed too quickly. It does not oppose for sport — it surfaces the smartest reason the proposed decision could be wrong, and the best alternative nobody argued for. It is uniquely positioned to catch groupthink, premature convergence, and false dichotomies — the failure where a BOARD agrees quickly and confidently on a decision that a rigorous devil's advocate would have shown to be one of several viable options, or the wrong one. It draws on inversion and steel-man reasoning patterns and on the record of alternatives already considered and rejected.
...
## Role Focus
Weight research toward: how a senior counter-thinker catches things this seat is *uniquely positioned* to catch — i.e., the failure modes the OTHER six BOARD seats systematically miss; the patterns this lens recognizes that generalists don't; the real-case precedent that grounds this seat's rationale.
...
What is the counter-thinker lens? What does it explicitly evaluate? What does it NOT evaluate (where it ABSTAINS)? What separates a great application of this lens from a mediocre one? What kind of decision benefits MOST from this lens being applied rigorously?
...
Failure modes the OTHER six BOARD seats systematically miss but counter-thinker should catch. Per mode:
- Name: `<short>`
  Pattern: `<what consistently happens>`
  Why other lenses miss it: `<one sentence>`
  Cost when it lands: `<concrete impact>`
  counter-thinker's catch: `<what counter-thinker sees that surfaces it>`

### §5 HARD-STOP TRIGGERS — 4-8 triggers
Conditions under which counter-thinker MUST emit `HARD-STOP: must be human` regardless of vote math. Per trigger:
...
- Case: `<company / project — short label>`
  ...
  Lesson: `<what this teaches the counter-thinker lens>`
  Source: `<link>`
...
## Source Quality
1. **PRACTITIONER** — essays on decision-making, red-teaming, devil's-advocacy, and inversion; post-mortems where groupthink or premature convergence caused failure; retrospectives on decisions that a steel-manned alternative would have changed.
2. **CASE_STUDY** — documented company-decision retrospectives (Stripe Press, Shopify Engineering, AWS post-mortems, Cloudflare blog, famous strategic-misjudgment case studies).
3. **OFFICIAL** — methodology canonical sources on inversion, pre-mortems, red-team process, and structured dissent where relevant.
4. **BOOK** — books authored by people known for rigorous contrarian / inversion-based evaluation at credible scale.
