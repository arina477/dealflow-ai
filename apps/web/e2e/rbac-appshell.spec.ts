...
test.describe('S1: compliance user — invite+signup → lands on / with AppShell', () => {
  ...
  test('after accept-invite: lands on / (NOT bounced to /login), AppShell renders, identity + role visible', async ({
    page,
  }) => {
    ...
    expect(currentUrl, 'Must NOT be on /login after successful invite acceptance').not.toMatch(
      /\/login/
    );
...
test.describe('S2: role-aware nav — compliance user nav set', () => {
  ...
  test('compliance: sees Dashboard + Compliance; does NOT see Mandates / Sourcing / Team / Settings', async ({
    page,
  }) => {
...
test.describe('S2: role-aware nav — advisor user nav set', () => {
  ...
  test('advisor: sees Dashboard + Mandates + Compliance; does NOT see Sourcing / Team / Settings', async ({
    page,
  }) => {
    await acceptInviteInBrowser(page, token, TEST_PASSWORD);
    // toHaveURL matches against the full URL; ensure we are at root (not /login).
...
test.describe('S5: login failure — wrong creds → inline error, no redirect', () => {
  test('wrong password shows role="alert" and stays on /login', async ({ page }) => {
    ...
    await expect(page, 'Page must stay on /login after failed login (no redirect)').toHaveURL(
      /\/login/
    );
