...
describe.skipIf(shouldSkip)(
  ...
  () => {
    ...
    it('INV-1: resolve_invite() returns email + workspace_id WITHOUT the GUC set (no-GUC bypass)', async () => {
      if (!dbReachable) return;
    ...
    it('INV-2: full consume cycle — invite consumed + user created in workspace W under FORCE RLS', async () => {
      if (!dbReachable) return;
      // Seed invite in workspace W.
    ...
    it('INV-3: new user in workspace W can read W data and CANNOT read workspace X data', async () => {
      if (!dbReachable) return;
      ...
      await withWorkspace(WS_W_ID, async (client) => {
        await client.query(`UPDATE invites SET consumed_at = now() WHERE token = $1`, [tokenHash]);
      });
    ...
    it('INV-4: resolve_invite() returns 0 rows for a consumed invite (no replay)', async () => {
      if (!dbReachable) return;
    ...
    it('INV-5: fault-killing — direct invites SELECT without GUC returns 0 rows (FORCE RLS active)', async () => {
      if (!dbReachable) return;
      ...
      try {
        await client.query('SET ROLE dealflow_app');
        await client.query('RESET app.workspace_id');
        const res = await client.query('SELECT email, workspace_id FROM invites WHERE token = $1', [
          tokenHash,
        ]);
        rows = res.rows;
      } finally {
