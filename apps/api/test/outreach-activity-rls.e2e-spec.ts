...
if (shouldSkip) {
  console.info(
    '[outreach-activity-rls] TEST_DATABASE_URL is not set — suite SKIPPED.'
  );
}
...
describe.skipIf(shouldSkip)(
  'B-2 write-path RLS e2e — wave-20 outreach-activity (5c12ac3a)',
  () => {
    beforeAll(async () => {
    ...
    it('OAE-9 (R4/SF5): create() appends exactly ONE outreach-activity-create audit entry + verifyChain ok', {
      ...
      let result: import('../src/modules/outreach-activity/outreach-activity.repository').OutreachActivityRow | undefined;
      try {
    ...
    it('OAE-11 (R4/SF5): updateStatus(completed) appends outreach-activity-status-transition + verifyChain ok', {
      ...
      expect(Number((await pool.query<{ count: string }>(`SELECT COUNT(*)::text AS count FROM audit_log_entries`)).rows[0]?.count ?? 0)).toBe(beforeCount + 1);
    ...
    it('OAE-13 (R4/SF5 rollback): audit throw → outreach_activity row NOT persisted', {
      ...
      const oaRepo = new OutreachActivityRepository(db);
      const authRepo = new AuthRepository(db);
      const svc = new OutreachActivityService(
        oaRepo,
        failingAuditSvc as never,
        authRepo
      );
